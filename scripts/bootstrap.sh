...
case "$(uname -s)" in
  Linux) NODE_OS=linux; NODE_ARCHIVE=xz ;;
  Darwin) NODE_OS=darwin; NODE_ARCHIVE=gz ;;
...
if [ -x "$NODE_DIR/bin/node" ]; then
  ...
  TARBALL="node-$NODE_VERSION-$NODE_OS-$NARCH.tar.$NODE_ARCHIVE"
  $NICE curl -fsSL --retry 2 --connect-timeout 20 "https://nodejs.org/dist/$NODE_VERSION/$TARBALL" -o "$RT/$TARBALL"
  [ -s "$RT/$TARBALL" ] || { log "FATAL: could not download node (set NODE_VERSION / check network)"; exit 1; }
  mkdir -p "$NODE_DIR"
  tar -xf "$RT/$TARBALL" -C "$NODE_DIR" --strip-components=1
  rm -f "$RT/$TARBALL"
...
if "$VENV/bin/python" -c "$DEPS" 2>/dev/null; then
  ...
  PYBASE="${DANUS_BOOTSTRAP_PYTHON:-$(command -v python3 || true)}"
  [ -n "$PYBASE" ] || { log "FATAL: no python3 on PATH to build the venv"; exit 1; }
  "$PYBASE" -c 'import sys; sys.exit(0 if sys.version_info >= (3, 10) else 1)' \
    || { log "FATAL: Python 3.10+ required; set DANUS_BOOTSTRAP_PYTHON (macOS: scripts/bootstrap-macos.sh)"; exit 1; }
...
export DANUS_NODE=$(printf '%q' "$NODE_DIR/bin/node")
export DANUS_NODE_BIN=$(printf '%q' "$NODE_DIR/bin")
export DANUS_CODEX_JS=$(printf '%q' "$CODEX_JS")
export DANUS_VENV=$(printf '%q' "$VENV")
ENV
